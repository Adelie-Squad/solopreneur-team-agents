...
/** Create a single adapter from MESSENGER env var (v1.2.0+ enforces single platform). */
...
/**
 * Returns a single-element adapter array (v1.2.0+ enforces one messenger per
...
export async function createAdapters(platforms?: string): Promise<MessengerAdapter[]> {
  ...
  if (raw.includes(",")) {
    console.log(
      `[Bot] MESSENGER contains multiple values ("${raw}"). v1.2.0+ supports one messenger per workspace — using "${resolvePlatform(raw)}".`
    );
  }
