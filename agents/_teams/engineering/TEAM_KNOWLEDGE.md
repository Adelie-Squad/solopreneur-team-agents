...
#### 테스트 피라미드
```
        ╱ E2E ╲           ← 적게, 핵심 플로우만 (Playwright)
       ╱ 통합    ╲         ← 중간, API/DB 연동 (Supertest)
      ╱  단위      ╲       ← 많이, 비즈니스 로직 (Vitest)
     ╱  정적 분석    ╲     ← 항상, TypeScript strict + ESLint
```
...
#### Go/No-Go 기준
- **Hard Gate** (필수 통과): Critical 버그 0건, 핵심 플로우 정상, 보안 취약점 없음, HTTPS 적용
- **Soft Gate** (권장): 성능 기준 충족, 접근성 Level A, 반응형 정상, 에러 모니터링 설정
...
| 항목 | 점검 내용 | 우선순위 |
|------|----------|---------|
| 입력 검증 | XSS, Injection, 길이 제한 | P0 |
| 인증/인가 | 토큰 관리, 권한 최소화 | P0 |
| 시크릿 관리 | .env 보호, 코드에 키 노출 금지 | P0 |
| 의존성 | npm audit, 취약 패키지 업데이트 | P1 |
| 출력 검증 | AI 출력에 민감 정보 포함 여부 | P1 |
| 로깅 | 민감 정보 로그 제외, 감사 로그 | P2 |

#### 안전 원칙
```
최소 권한: 필요한 권한만 부여
인간 확인: 파괴적 작업 전 반드시 사람 확인
방어적 코딩: 모든 외부 입력은 신뢰하지 않음
감사 추적: 모든 AI 실행 기록 보존
```
