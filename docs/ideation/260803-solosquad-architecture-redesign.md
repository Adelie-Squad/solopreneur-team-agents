...
> 착수 전 결정해야 할 것은 **§0.0.7 미결 대장**(14건)과 **§0.0.8 블로커**에 있다.
...
| id | 미결 | 등급 | 차단하는 단계(§F′) | 권장 |
|---|---|:---:|---|---|
| **M1** | `migrations/` 43스크립트 + 기존 워크스페이스 처분 | 🔴 | **[0] 골격** | ⓑ 1회성 컨버터 |
| **M2** | npm 레지스트리 `solosquad` 1.4.3 처분 | 🟡 | 없음 (배포 중단 시점) | deprecate + 게시 유지 |
| **M3** | 릴리스 번호 체계 | 🟡 | [0] 골격 (문서 명명) | — |
| **M4** | Python 스택 확정 | 🔴 | **[0] 골격** | 아래 §M4 초안 |
| **M5** | `curl` 부트스트랩 + 사용 측정 설계 | 🟡 | [6] 배포 | 옵트인 |
| **M6** | Discord/Slack 재도입 시점·언어 | 🟡 | 없음 (클라우드 착수 시) | §0.0.4 3조건 |
...
### M1. `migrations/` 43스크립트 + 기존 워크스페이스 처분 🔴

**⚠️ §0.0.3 의 제거 실측표는 "전량 폐기"를 전제로 7,839 LOC 을 계산했다. 미확정이다.**

| 안 | 내용 | 이식 범위 | 본인 org 데이터 |
|---|---|---|---|
| ⓐ | 전량 폐기 + v2.0 을 새 baseline. 기존 사용자 재설치 | −7,839 | 🔴 유실 |
| **ⓑ** | 43개는 버리고 `1.4.3 → v2.0` **1회성 컨버터 1개만 Py 로 신규 작성** | −7,839 / +1 | ✅ 보존 |
| ⓒ | 43개 전량 Py 포팅 | 0 | ✅ 보존 |

**ⓑ 권장 근거** — 43개 체인이 비쌌던 이유는 npm 배포 34버전과의 계약이었고 그 계약은 소멸했다(§0.0.2).
그러나 **본인이 실사용 중인 org 데이터**(`memory` jsonl · `workflows/` · `goals/` · `sessions/`)는 살아 있고,
그것은 체인이 아니라 **단발 변환 1개**로 충분하다.

**부수 결정** — 컨버터가 다룰 범위: `<org>/` 하위 전부인가, 메모리만인가.
Discord 관련 상태(`user-registry` · 채널 매핑 · 승인 카드 이력)는 **변환 대상에서 제외**된다(결정 4).
...
2. **분석 스택의 도입 범위** — `pandas`/`scipy`/`sklearn` 중 어디까지. 결정 6(데이터 분석·시뮬레이션)의
   실질이 여기서 갈린다. 무겁게 잡으면 git clone 설치 시간이 는다
3. **비동기 모델** — `asyncio` 전면인가, 동기 + 스레드풀인가. 하네스가 서브프로세스를 다루므로
   §C1 계약 설계 전에 정해야 한다
...
### M6. Discord/Slack 재도입 시점·언어 🟡

**해제 요건은 §0.0.4 의 3조건.** 결정 시점은 클라우드 배포(구 `v1.6.0` 계획) 착수 시.

**부수 결정** — 제거되는 코드(`messenger/` 3,624 + `bot/` 결합부 ~734 + `cli/` 561 = **4,919 LOC**)를
어디에 보존하는가. git 히스토리로 충분한가, 아니면 **태그 또는 참조 브랜치**를 남기는가.
재도입 시 이 코드가 사양서 역할을 하므로 **찾기 쉬운 위치를 명시**해 두는 편이 낫다.
...
## 0.0.8 착수 전 필수 결정 (블로커)

§F′ 각 단계를 시작하려면 다음이 먼저 닫혀야 한다.

```
[0] 골격   ← M1 (스키마 baseline) · M4 (스택) · M3 (명명, 약하게)
[2] 컨텍스트 ← M12 (세션 비용 배수 — 실측이면 [2] 안에서 수행 가능)
[3] 실행   ← M9 (토폴로지) · M10
[6] 배포   ← M5 · M2
```

**즉 지금 당장 필요한 것은 M1 · M4 두 건뿐이다.** 나머지는 해당 단계 직전까지 열어둬도 된다.
