...
> 착수 전 결정해야 할 것은 **§0.0.7 미결 대장**(14건, 이 중 1건 닫힘 · 3건 부분 확정)에 있다.
...
#### 4차 결정 (2026-08-10 — 메신저 복귀 시점이 닫힘)
...
> ADR-001 §5 의 재도입 요건 1번이 이미 *"클라우드 배포가 착수되어 **Node 전제조건을 배포자만
...
> **상태 열 갱신 2026-08-08(3차)** — ✅ 닫힘 · 🔷 부분 확정(잔여 있음) · ⬜ 열림
...
| **M8** | `docs/policy/adr-001` 존치 여부 | ⬜ | 🟡 | 없음 | 존치 |
...
⑴ ADR-001 §5 요건 1(*Node 전제조건을 배포자만 지는 형태*)을 **만드는 작업이 곧 클라우드 배포**이고,
...
⬜ **잔여 — 언어 1건.** `v2.1.0` 착수 시 ADR-001 §5-2 원칙(*플랫폼 실시간 I/O·어댑터·UI = TS*)으로
판단한다. 결정 9·ADR §5 가 메신저를 **예정된 TS 수요**로 세어 두었으므로 TS 쪽으로 기울어 있으나,
...
### M8. `docs/policy/adr-001-orchestration-language.md` 존치 여부 🟡

이 문서 §0.0 과 내용이 겹친다. 그러나 **층이 다르다** —
ideation 은 논의 맥락, policy 는 권위(§A3-1 이 요구한 박제).
**존치 권장.** 단 중복 유지비를 줄이려면 ADR 은 결론·재론 금지 조항만 두고
근거는 이 문서를 참조하는 형태로 축약한다.
