...
| **M6** | Discord/Slack 재도입 시점·언어 | 🔷 | 🟢 | 없음 | **재도입 확정**(결정 9), 시점·언어만 잔여. 형식 기록은 결정 12 로 닫힘 |
...
남은 것은 **시점과 언어**뿐이며, 그때 지켜야 할 형태는 §0.0.4 의 3요건이다.
예상 시점은 클라우드 배포(구 `v1.6.0` → 현 `v2.1.0`) 착수 시.
...
⬜ **잔여** — 형식 문서를 어디에 둘 것인가(`docs/policy/` vs `docs/ideation/`), 제거 커밋 시점에 작성.
...
> | v2.1.0 — 클라우드·컨테이너 격리 | 🟡 **유지 · 후행** — Docker·메신저 재도입이 여기 모임 (결정 4·5) |
