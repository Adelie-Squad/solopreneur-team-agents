...
**단 지금은 적용되지 않는다.** 우리 코드에서 "TS 적합 영역"은 `messenger/` 뿐이었고,
결정 4 로 그것이 제거되어 **TS 잔존부가 0** 이다. 따라서 v2.0 은 폴리글랏이 아니라 **Python 단일 언어**다.
...
**새 미결 (결정 필요):**
1. **`migrations/` 43스크립트의 처분** — 위 §0.0.3 은 "폐기 + 새 baseline"을 **전제로 계산**했으나 미확정.
   대안: ⓐ 전량 폐기(기존 사용자 재설치) ⓑ 기존 org 데이터(memory jsonl · workflows · goals · sessions)
   이관용 **1회성 컨버터 1개만 신규 작성** ⓒ 전량 Py 포팅. — **ⓑ 권장**
2. **릴리스 번호** — `v2.0` 인가, 아니면 `1.x` 계보를 끊고 별도 체계인가
3. **Python 스택 확정** — 제안: `uv` · `typer`(CLI) · `pydantic`(스키마) · `rich`+`questionary`(온보딩) ·
   `APScheduler`(node-cron 대체) · 표준 `sqlite3`(FTS5 내장) · `PyYAML`
4. **`curl` 부트스트랩 + 사용 측정 도구**의 설계 시점과 수집 항목·옵트인 방식
...
|---|---|:---:|---|---|
| **A1** | 오케스트레이션 언어 TS/Py/폴리글랏 | 🔴 | ~~**TS 유지 + 최소 폴리글랏**(분석·시뮬만 Py subprocess). 언어를 게이트에서 강등~~ | 🔴 **반전 — Python 단일 언어 전면 재작성.** 폴리글랏 아님(§0.0.4) |
...
| **B1** | Chief 존재 vs PM 흡수 | 🔴 | **Chief 를 라우터로 축소**(ⓓ) + fast-path. 2단 기획은 A/B 로만 정당화 | 🟢 **유효 · 강화** — 재작성이므로 중간안(ⓓ)의 "비용이 크다" 제약이 사라짐. 전면 통폐합 재검토 가능 |
...
> **요약:** 18건 중 **반전 2 (A1·A2)** · **부분 변경 3 (B5·D1·D2)** · **승격 3 (B4·B6·C1)** ·
> **나머지 10 은 그대로 유효**하다. 토폴로지(B)·하네스(D) 안건이 언어 중립이라는 §A1 의 판정이
> 역설적으로 여기서 증명된다 — **언어가 뒤집혀도 안건의 5/6 이 살아남았다.**
...
> | v1.6.0 — 클라우드·컨테이너 격리 | 🟡 **유지 · 후행** — Docker·메신저 재도입이 여기 모임 (결정 4·5) |
...
> 🔴 **결정 반영(2026-08-07) — 아래 7건 중 3건 소멸, 1건 축소, 3건 존속. 신규 4건은 §0.0.6.**
>
> | # | 원 질문 | 처분 |
> |---|---|---|
> | 1 | 하이브리드 spawn 을 MCP 로 제공? | ⬜ **소멸** — Py 러너가 spawn 을 직접 소유하므로 하이브리드 자체가 불필요 |
> | 2 | Educational Nudge 소유자 (Chief vs PM) | 🟡 **존속** |
> | 3 | org PRD 를 누가 쓰나 | 🟡 **존속** |
> | 4 | 세션 폭증의 실제 비용 배수 | 🟡 **존속 · 중요도 상승** — 결정 6 의 장기 사이클 작업이 이 위험에 직결 |
> | 5 | Tier-2 를 정말 만들 것인가 | 🟡 **축소 존속** — 언어 논점은 빠지고 "도구를 직접 구현하는 비용" 만 남음 |
> | 6 | 승격 게이트의 최소 표본(2 repo)이 1인에게 현실적인가 | 🟡 **존속** |
...
- `docs/policy/adr-001-orchestration-language.md` — 이 결정의 정식 ADR
