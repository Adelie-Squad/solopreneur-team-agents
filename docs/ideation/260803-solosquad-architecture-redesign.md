...
> 착수 전 결정해야 할 것은 **§0.0.7 미결 대장**(14건, 이 중 3건 부분 확정)에 있다.
...
> (`docs/prd/v1.x-dashboard-interaction.md`)가 계획에 있으므로 **목적지는 2언어**다.
...
> **상태 열 갱신 2026-08-07(2차)** — ✅ 닫힘 · 🔷 부분 확정(잔여 있음) · ⬜ 열림

| id | 미결 | 상태 | 등급 | 차단하는 단계(§F′) | 잔여 / 권장 |
|---|---|:---:|:---:|---|---|
| **M1** | `migrations/` 43스크립트 + 기존 워크스페이스 처분 | 🔷 | 🟡 | **없음** (하향) | 체인 폐기·재설치 **확정**. **org 데이터 처분만 잔여** |
| **M2** | npm 레지스트리 `solosquad` 1.4.3 처분 | ⬜ | 🟡 | 없음 (배포 중단 시점) | deprecate + 게시 유지 |
| **M3** | 릴리스 번호 체계 | ⬜ | 🟡 | [0] 골격 (문서 명명) | — |
| **M4** | Python 스택 확정 | 🔷 | 🔴 | **[1] 계약** (하향) | 10종 + pandas **확정**. **토크나이저 · 동시성 모델 잔여** |
...
| 안 | 내용 | 영향 |
|---|---|---|
| `v2.0.0` | semver 계보 유지, major 로 격변 표시 | `docs/prd/v2.0_*.md` · CHANGELOG 연속 |
| `0.1.0` 재시작 | npm 계보를 끊고 새 제품으로 | 과거 34버전과 단절 명시 |
| 날짜 기반 | §B3 의 org PRD 규약(`prd-YYYY-MM-DD-slug`)과 동형 | ⚠️ §B3 은 **repo = semver** 를 명시했으므로 충돌 |

**영향 범위** — `docs/prd/` 파일명 규약(§B3-5 도그푸딩) · `CHANGELOG.md` · `manual/` 2종 버전 헤더 ·
기존 `npm run docs-check` 4-docs 게이트의 후신.
...
2. **동시성 모델** 🔴 — 하네스가 `claude` 서브프로세스를 여러 개 동시에 다루므로
   **§C1 계약 설계(`[1]`) 전에 정해야 한다.** 선택지는 아래 3안.

   | 안 | 형태 | 장점 | 대가 |
   |---|---|---|---|
   | ⓐ 동기 + 스레드풀 | `ThreadPoolExecutor` | 함수가 평범 — 라이브러리 제약 없음 | 스레드당 ~8MB, 수천 개는 무리 |
   | ⓑ `asyncio` 전면 | 전 계층 `async def` | 경량, 스트리밍·웹서버에 자연스러움 | **전염(function coloring)** — `await` 는 `async` 안에서만 쓸 수 있어 위로 계속 번짐. 확정 스택 중 `sqlite3`·`PyYAML`·`questionary` 가 동기 전용이라 우회 필요 |
   | ⓒ 계층 분리 | 하네스만 async, 겉은 동기 함수로 포장 | 동시 실행 이득 + 코어는 평범한 Python. 대시보드(async 웹서버) 합류 시 코어 무수정 | 경계 규율을 지켜야 함 |

   판단 재료 — 우리 대기는 CPU 작업이 아니라 **서브프로세스 I/O 대기**이고 동시 세션은 수십 개
   규모다. 반면 결정 9(대시보드 예정)로 표면 쪽에 async 가 들어올 것은 확정이다.
   **이 절은 사용자 검토 대기 중이며 열어 둔다.**
...
예상 시점은 클라우드 배포(구 `v1.6.0` 계획) 착수 시.
...
## 0.0.8 착수 전 필수 결정 (블로커) — 2차 결정 반영

§F′ 각 단계를 시작하려면 다음이 먼저 닫혀야 한다.

```
[0] 골격    ← 없음 ✅            (M1·M4 가 2차 결정으로 해소되어 착수 가능)
              · M3 (명명)은 약한 의존 — 폴더/문서 이름만 잠정으로 두고 진행 가능
[1] 계약    ← M4-2 동시성 모델 🔴  ← 유일한 실질 블로커
[2] 컨텍스트 ← M12 (세션 비용 배수 — 실측이면 [2] 안에서 수행 가능)
[3] 실행    ← M9 (토폴로지) · M10
[6] 배포    ← M5 · M2
```

**변화** — 초판은 M1·M4 두 건이 `[0]` 을 막는다고 봤다. 2차 결정으로 둘 다 해소됐다.

- **M1** — 마이그레이션 체인 폐기가 확정(결정 11)됐고, 남은 org 데이터 변환기는
  **새 구조 확정 후 마지막에** 만들면 되므로 선행 조건이 아니다
- **M4** — 스택 10종 + `pandas` 확정(결정 13). 잔여 2건 중 토크나이저는 `[2]` 이후에나
  쓰이고, **동시성 모델만 `[1]` 을 막는다**

> **즉 `[0] 골격`은 지금 착수할 수 있고, `[1] 계약` 전까지 M4-2 동시성 모델 하나만 닫으면 된다.**
> `[1]` 은 결정 10 에 따라 하네스 계약과 **코어↔표면 JSON 경계**를 함께 설계하므로,
> 동시성 선택이 그 경계 모양에 직접 영향을 준다.
