...
### 변경 예정(npm v1.2.0) — 한 워크스페이스 = 한 메신저

v1.2.0부터는 워크스페이스당 **단일 메신저 플랫폼**으로 단순화됩니다. `MESSENGER=discord,slack` 같은 복수 지정은 제거되고, 동시 운영이 필요한 사용자는 **워크스페이스를 여러 개** 두는 방식으로 바뀝니다. 자세한 이유와 마이그레이션은 `docs/v1.2.2-terminology-layout.md` §3.6과 `docs/v1.2.3-migration-process.md` 참조. (문서 파일명의 `v1.2.2-`/`v1.2.3-` 접두사는 작업 블록 라벨; 실제 npm 배포는 `v1.2.0`.)
