...
src/
  cli/
    ...
    cron.ts (index)              → cron 데몬(스케줄러) 시작 + cron 잡 관리
    status.ts                    → 대시보드
    update.ts                    → 자체 업데이트 + 마이그레이션 감지
    doctor.ts                    → 환경/토큰/레이아웃 진단
    run-cron.ts                  → cron 잡 수동 실행
  ...
  scheduler/
    crons.ts                     → 5개 cron 잡 정의 (cron 표현식)
    memory.ts                    → JSON 블록 추출 → JSONL append (중복 방지)
    index.ts                     → node-cron 등록 + org cwd 해석 후 cron 잡 실행
  migrations/
...
Layer 1: Organization (<workspace>/<org>/)
  ...
  memory/cron-logs/  (모든 cron 잡 실행 결과)
...
- **notifier 모드**: 스케줄러가 cron 잡 결과를 채널로 전송
...
resolveOrgCwd → repo 또는 org 루트에서 Claude 실행
  ...
  → <org>/memory/cron-logs/{id}-{timestamp}.md (모든 실행 기록)
  → 메신저 채널 전송 (연결된 플랫폼)
```

**중요:** cron 잡은 특정 repo 에서 실행될 수 있지만 **메모리 저장은 항상 org 레벨**. 여러 repo 를 아우르는 의사결정·시그널이 단일 소스에 축적.
...
#### 13.6.30 v1.3.7 — workflow/goal/cron 작성법 내재화 + 워크플로 재구성 (2026-06-24)

v1.3.6 의 작성권위 패턴을 workflow·goal·cron 으로 확장 + 번들 워크플로 재구성 + goal validator. **단일 코어** `skills/skill-core/core.md`→`primitive-core.md`: §0 primitive 분류·단위/조립 관계(skill·agent=워크스페이스 베이스, workflow·goal·cron=org 조립물로 베이스 skill·agent 참조 — 조립이지 발명 아님) · §1 universal 철학 · §2 암묵지 추출 인터뷰(초안앵커 4-mode: 명시/마이그레이션/대화/마이닝) · §3 SKILL.md 포맷(skill·agent, 구 core 흡수, §N→§3.N 6참조 repoint) · §4 조립 포맷 + **워크플로 본질 원칙**(목표·근거·방법→결론→핸드오프; 단순 행위면 skill) + **기획 3대 편향 가드**(자기부정·학습편향·확증편향) · §5 수용 rubric. **매니저** workflow/goal/cron 작성권위 격상·skill 인터뷰 retrofit·`lifecycle.md` 4-mode·Chief `[creation_case:N]` 마커(마이그레이션 1급=분석→초안→WHY 추출→agent-ref 해소). **워크플로 재구성** 레거시 4종(discovery-cycle·pmf-validation·autoplan-pm·weekly-retro)+monolithic problem-definition 폐기; scqa·five-whys·tdcc skill→워크플로 흡수, mece·xyz-hypothesis skill 유지; idea-refinement·market-research·kpi-check 본질원칙 재서술(kpi-check=목표 정렬 게이트). **goal validator** `src/bot/goal-validate.ts`(metric provenance·pipeline agent 실존·termination·composite guardrail)→`solosquad goal validate`+`solosquad validate` 5종. published `1.1.0-to-1.2.6` 마이그레이션의 obsolete problem-definition seed+패키지 무결성 가드 제거(user-authorized immutable-migration 예외, v1.3.5 B-D1 선례 동형). 연속성 마이그레이션 `1.3.6-to-1.3.7`. bundle-only·사용자 데이터 무변경. 981 test green.

자세히: `docs/prd/v1.3.7-workflow-goal-cron-authoring-internalization.md`
...
어휘로 부르던 혼선을 제거하고 CLI·코드·디렉토리·데이터 경로를 cron 으로 일원화.

- **개념 통합** — routine(빌트인 잡) + schedule(사용자 잡) → 단일 명사 cron.
- **코드 개명** — `scheduler/routines.ts`→`crons.ts`, `schedule-def.ts`→`cron-def.ts`,
  `schedule-validate.ts`→`cron-validate.ts`, `cli/schedule.ts`→`cli/cron.ts`,
  `cli/run-routine.ts`→`cli/run-cron.ts`. 식별자 `ROUTINES`→`CRONS`, `RoutineConfig`→`CronConfig`,
  `ScheduleDef`→`CronDef`, `getSchedulesDir`→`getCronsDir`, 에러코드 `SCHED_*`→`CRON_*`.
- **CLI** — 단일 `cron` 그룹(`start`/`run`/`list`/`new`/`show`/`validate`) — 구
  `schedule`(데몬)·`schedules`(관리)·`run-routine`(수동) 대체.
- **경로** — 번들 디렉토리 `schedules/`→`crons/`, 데이터 경로
  `.solosquad/{schedules,routines}`→`.solosquad/crons`, `memory/routine-logs`→`memory/cron-logs`.
- **보존** — `scheduler` 서브시스템명·`startScheduler`·node-cron API·아카이브 이벤트 enum
  `routine_log`(데이터 계약)·과거 migration 스크립트 경로는 변경 없음.
- **무중단 migration** — 1.3.2→1.3.3 migration 이 기존 워크스페이스 디렉토리를 자동 이전,
  `getCronsDir` 의 레거시 override read 로 무중단 전환.

자세히: `docs/prd/v1.3.3-cron-terminology.md`
...
자세히: `docs/prd/v1.3.2-asset-managers-validate.md`
