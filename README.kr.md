...
내부 아키텍처 · 릴리스 계획 · 결정 이력은 [`docs/roadmap.md`](docs/roadmap.md) + [`docs/architecture.md`](docs/architecture.md) 참조.
...
- **v0.8.1 Security & Lifecycle Pair** — npm audit 7건 → 0. archive 페어 완결 (`solosquad import` + `archive verify/info/list`). `docs/policy/schema-stability.md` 신설 — 6개 schema_version 의 bump 룰 + deprecation 기간 박제. v1.0 정식 출시의 *전제 항목*.
...
- VPS + systemd (자세히는 [`manual/cloud-deployment.md`](manual/cloud-deployment.md))
...
v1.0 이 정식 출시 마일스톤으로 안정 API 보장이 시작되었습니다. 출시된 + 예정된 마일스톤 (전체 이력은 [`CHANGELOG.md`](CHANGELOG.md), 결정 로그는 [`docs/roadmap.md`](docs/roadmap.md) §6):
...
| **v0.8.1 (출시)** | **Security & Lifecycle Pair** | npm audit 7건 → 0; archive 페어 완결 (`solosquad import` + `archive verify/info/list`); `docs/policy/schema-stability.md` (6 schema_version 의 bump 룰); 25 SKILL.md `schema_version: 1` 백필 |
...
| **v1.0.0 (출시)** | **정식 출시** | 안정 API 보장 · 42 CLI surface freeze · `docs/policy/schema-stability.md` 공개 약속 발효 · Discord 단일 메신저 (Slack post-v1.0 슬롯) |
...
결정 로그: [`docs/roadmap.md`](docs/roadmap.md) §6.
...
docs/
  ...
  roadmap.md                      → 마스터 로드맵 + 결정 로그 (repo 계층, living)
  architecture.md                 → 내부 시스템 설계 (repo 계층, living)
  prd/                            → repo별 release PRD (repo 계층) + INDEX.md
  reports/ · ideation/            → 제품 조사/탐색 (org 계층) + INDEX.md
  manual/cloud-deployment.md        → VPS + systemd 셋업
  schema-stability.md                → v0.8.1 — schema_version bump 룰 + deprecation 기간
...
솔로 파운더에 *과한 엔지니어링*으로 명시적 거부된 항목: 3-repo 물리 분리, LangGraph v3 그래프 오케스트레이션, MCP 기반 내부 SKILL 레지스트리, Vector + Graph DB 하이브리드, 자동 머지. 사유는 [`docs/roadmap.md`](docs/roadmap.md) §6 참조.
